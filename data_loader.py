...
class SampleLevelMTTDataset(Dataset):
    ...
    def set_mode(self, mode):
        ...
        if mode == 'train':
            self.annotation_file = Path(config.BASE_DIR) / 'train_4_tags_annotations_final.csv'
        
        elif mode == 'valid':
            self.annotation_file = Path(config.BASE_DIR) / 'valid_4_tags_annotations_final.csv'

        elif mode == 'test':
            self.annotation_file = Path(config.BASE_DIR) / 'test_4_tags_annotations_final.csv'
